...
<body>
	...
	<c:forEach items="${boardPage.boardList}" var="boardVO">
		<div class="card mb-4">
			...
			<div class="card-body">
				<div class="row">
					<div class="col-md-10">
						<h2 class="card-title">${boardVO.boardNum}.${boardVO.title}</h2>
					</div>
			...
			<div class="card-footer text-muted">
				<div class="row">
					...
					<div class="col-md-5">
						<div class="dropdown">
							By
							<c:if test="${sessionScope.loginId == boardVO.writer}">${boardVO.writer}</c:if>
							<c:if test="${sessionScope.loginId != boardVO.writer}">
								...
								<ul class="dropdown-menu" role="menu"
									...
									<li><input type="hidden" name="otherProfileInfo"
										value="${boardVO.writer}" /> <a class="otherProfileBtn"
										href="#">profile</a></li>
	...
	<ul class="pagination justify-content-center mb-4">
		<c:if test="${boardPage.currentPage <= boardPage.startPage}">
		<c:if test="${boardPage.startPage != 1}">
			<li class="page-item"><a class="page-link"
				href="board.do?page=${boardPage.startPage -1 }">&larr; 이전</a></li>
		</c:if>
		</c:if>
		<c:if test="${boardPage.currentPage > boardPage.startPage}">
			<li class="page-item"><a class="page-link"
				href="board.do?page=${boardPage.currentPage -1}">&larr; Newer</a></li>
		</c:if>
		<div>
			<!-- 	하단 페이지 링크 부분 -->
			<c:forEach begin="${boardPage.startPage}" end="${boardPage.endPage}"
				var="p" step="1">
				<c:if test="${boardPage.currentPage == p}">${p}</c:if>
				<c:if test="${boardPage.currentPage != p}">
		...
		<c:if test="${boardPage.currentPage < boardPage.endPage}">
			<li class="page-item"><a class="page-link"
				href="board.do?page=${boardPage.currentPage +1}">Older &rarr;</a></li>
		</c:if>
		<c:if test="${boardPage.currentPage >= boardPage.endPage}">
		<c:if test="${boardPage.endPage != boardPage.totalPage}">
			<li class="page-item"><a class="page-link" href="board.do?page=${boardPage.endPage + 1}">다음
					&rarr;</a></li>
		</c:if>
		</c:if>
	</ul>
